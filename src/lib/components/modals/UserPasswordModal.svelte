<script lang="ts">
	...
	export let title: string = 'Change Password';
	export let description: string = 'Changing password for';
	export let entityName: string = '';
	export let passwordLength: number = 20;
	...
	async function generateRandomPassword(): Promise<void> {
		try {
			const generatedPassword = await generateAndCopyPassword(passwordLength);
	...
	$: buttonEnabled = !!newPassword.trim();
</script>
			...
			<div class="p-6">
				...
				<div class="mb-4">
					<p class="text-sm text-gray-600 dark:text-gray-400">
						{description}
						{#if entityName}
							<strong class="text-gray-900 dark:text-white">{entityName}</strong>
						{:else if username}
							<strong class="text-gray-900 dark:text-white">{username}</strong>
						{/if}
					</p>
				</div>
