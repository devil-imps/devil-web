<script lang="ts">
	...
	function handleKeydown(event: KeyboardEvent) {
		if (event.key === 'Enter' && !loading && username.trim() && password.trim()) {
			handleSubmit();
		}
	}
</script>
		...
		>
			<div class="p-6">
				<div class="flex justify-between items-center mb-6">
					<h2 class="text-xl font-bold text-gray-900 dark:text-white">Add Statistics User</h2>
					{#if !loading}
						<button
							on:click={closeModal}
				...
				<form on:submit|preventDefault={handleSubmit} class="space-y-4">
					<div>
						...
						<input
							...
							id="username"
							bind:value={username}
							placeholder="Enter statistics username"
						...
						<p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
							Username for the statistics account
						</p>
					</div>
					...
					<div>
						...
						<p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
							Enter a secure password for the statistics account
						</p>
					</div>
				</form>
				...
				<div class="flex justify-end space-x-3 mt-6">
					...
					<button
						...
						class="btn-primary {loading || !username.trim() || !password.trim()
