<script lang="ts">
	...
	async function generateRandomPassword(): Promise<void> {
		try {
			...
			mysqlPassword = generatedPassword;
	...
	function handleSubmit() {
		if (mysqlUsername.trim() && mysqlPassword.trim()) {
			dispatch('submit', {
				user_name: `${mysqlSpecialUser}_${mysqlUsername.trim()}`,
	...
	function handleKeydown(event: KeyboardEvent) {
		if (event.key === 'Enter' && !loading && mysqlUsername.trim() && mysqlPassword.trim()) {
			handleSubmit();
		}
	}
</script>
		...
		>
			<div class="p-6">
				<div class="flex justify-between items-center mb-6">
					<h2 class="text-xl font-bold text-gray-900 dark:text-white">Add MySQL User</h2>
					{#if !loading}
						<button
							on:click={closeModal}
				...
				<form on:submit|preventDefault={handleSubmit} class="space-y-4">
					...
					<div class="flex justify-end space-x-3 pt-4">
						...
						<button
							type="submit"
							disabled={loading || !mysqlUsername.trim() || !mysqlPassword.trim()}
