<script lang="ts">
	...
	async function loadCollations() {
		try {
			const response = await apiGet('/pgsql/collations');
			// API returns { collations: string[] }, so extract the collations array
			const responseData = response as { collations?: string[] };
			collations = Array.isArray(responseData.collations) ? responseData.collations : [];
		} catch (error) {
			console.error('Failed to load collations:', error);
			collations = [];
			// Don't show toast for collation loading failure as it's optional
		}
	}
	...
	function handleSubmit() {
		if (databaseName.trim() && password.trim()) {
			const data: { database_name: string; password: string; collate?: string } = {
				database_name: databaseName.trim(),
				password: password.trim()
			};

			if (selectedCollation) {
				data.collate = selectedCollation;
			}
		...
		>
			<div class="p-6">
				<div class="flex justify-between items-center mb-6">
					<h2 class="text-xl font-bold text-gray-900 dark:text-white">Add PostgreSQL Database</h2>
					{#if !loading}
						<button
							on:click={closeModal}
				...
				<form on:submit|preventDefault={handleSubmit} class="space-y-4">
					<div>
						...
						<p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
							Choose a unique name for your PostgreSQL database
						</p>
					</div>
					...
					<div>
						...
						<p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
							Choose a collation for your database (optional, defaults to system default)
						</p>
					</div>
				...
				<div class="flex justify-end space-x-3 mt-6">
					...
					<button
						...
						title={loading
							? 'Creating database...'
							: !databaseName.trim() || !password.trim()
